...
(async () => {
  ...
  try {
    ...
    await page.clock.runFor(10000);
    assert.ok(await page.evaluate(() => novaAnimationCalls.filter(call => call.eye).length) >= overviewCount + 2, 'Nova continues blinking after returning home');
    await page.emulateMedia({ reducedMotion: 'reduce' }); await page.clock.runFor(100);
    const reducedCount = await page.evaluate(() => novaAnimationCalls.filter(call => call.eye).length);
    await page.clock.runFor(20000);
    assert.equal(await page.evaluate(() => novaAnimationCalls.filter(call => call.eye).length), reducedCount);
    assert.equal(await page.locator('.mascot').evaluate(el => getComputedStyle(el).animationName), 'none');
    await page.emulateMedia({ reducedMotion: 'no-preference' }); await page.click('#appearance');
    await page.clock.runFor(20000); assert.equal(await page.evaluate(() => novaAnimationCalls.filter(call => call.eye).length), reducedCount);
    if (!wasSimple) await page.click('#appearance');
    assert.deepEqual(errors, []);
    console.log('PASS: exact 10-second blink cadence, expressive smooth gaze, silent clicks, persistent companion dock, reduced motion and simple appearance.');
